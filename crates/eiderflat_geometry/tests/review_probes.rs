...
fn mirror_arc_keeps_radius_flips_center() {
    ...
    if let Curve::Arc(a) = t.apply_curve(&arc) {
        ...
        assert!((a.included_angle() - std::f64::consts::FRAC_PI_2).abs() < 1e-6,
            "included angle changed, expected a quarter arc: {}", a.included_angle());
        ...
        assert!((ms_x - oe_x).abs() < 1e-6 && (ms_y + oe_y).abs() < 1e-6,
            "mirrored start ({},{}) is not the reflection of original end ({},{})", ms_x, ms_y, oe_x, oe_y);
        assert!((me_x - os_x).abs() < 1e-6 && (me_y + os_y).abs() < 1e-6,
            "mirrored end ({},{}) is not the reflection of original start ({},{})", me_x, me_y, os_x, os_y);
