...
pub fn offset_curve(curve: &Curve, dist: f64) -> Curve {
    match curve {
        ...
        Curve::Nurbs(nc) => {
            offset_nurbs(nc, dist)
        }
        Curve::Poly(pc) => {
            offset_polycurve(pc, dist)
        }
        Curve::Ellipse(_) | Curve::Rational(_) => {
...
fn offset_polycurve(pc: &crate::primitives::PolyCurve, dist: f64) -> Curve {
    ...
    if n == 0 { return Curve::Poly(Box::new(pc.clone())); }
...
fn miter_join(offs: &mut [Curve], i: usize, k: usize) {
    ...
    let x = infinite_line_intersection(a, b)
        .unwrap_or(((a.1.0 + b.0.0) * 0.5, (a.1.1 + b.0.1) * 0.5));
...
fn set_line_p0(c: &mut Curve, x: (f64, f64)) {
    if let Curve::Line(l) = c { l.p0 = Point2d::from_f64(x.0, x.1); }
}
fn set_line_p1(c: &mut Curve, x: (f64, f64)) {
    if let Curve::Line(l) = c { l.p1 = Point2d::from_f64(x.0, x.1); }
}
...
fn infinite_line_intersection(
    ...
    let imp = |((x0, y0), (x1, y1)): ((f64, f64), (f64, f64))| (y0 - y1, x1 - x0, x0 * y1 - x1 * y0);
    let (a1, b1, c1) = imp(a);
    let (a2, b2, c2) = imp(b);
    let det = a1 * b2 - a2 * b1;
...
fn offset_nurbs(nc: &NurbsCurve, dist: f64) -> Curve {
    ...
    let data: Vec<Point2d> = params.iter().map(|&t| {
...
pub fn interpolate_nurbs(data: &[Point2d], weights: &[f64]) -> Option<NurbsCurve> {
    let m = data.len();
    if m < 2 || weights.len() != m { return None; }
    let params: Vec<f64> = (0..m).map(|k| k as f64 / (m - 1) as f64).collect();
    ...
    solve2(&mut mat, &mut qx, &mut qy).map(|()| {
        let control = qx.iter().zip(&qy).map(|(&x, &y)| Point2d::from_f64(x, y)).collect();
...
pub fn refit_nurbs_subcurve(nc: &NurbsCurve, a: f64, b: f64) -> NurbsCurve {
    ...
    let data: Vec<Point2d> = (0..m).map(|k| {
...
fn solve2(a: &mut [Vec<f64>], b1: &mut [f64], b2: &mut [f64]) -> Option<()> {
    ...
    for col in 0..n {
        ...
        a.swap(col, piv); b1.swap(col, piv); b2.swap(col, piv);
...
mod tests {
    ...
    fn offset_spline_stays_a_spline() {
        ...
        assert_eq!(nc.control.len(), cvs.len(), "control-vertex count preserved");
        ...
        for k in 0..m {
            ...
            assert!((d - dist).abs() < 1e-6, "node {k}: offset distance {d}, want {dist}");
        }
    }
    ...
    fn offset_square_polycurve_miters_corners() {
        ...
        let pc = match &off { Curve::Poly(pc) => pc, o => panic!("expected Poly, got {:?}", o) };
        ...
        assert!(near(s0, (1.0, 1.0)) && near(e0, (3.0, 1.0)), "bottom edge {s0:?}->{e0:?}");
        for j in 0..4 {
            let (_, end) = seg_ends(&pc.segments[j]);
            let (start_next, _) = seg_ends(&pc.segments[(j + 1) % 4]);
            assert!(near(end, start_next), "corner {j} discontinuous: {end:?} vs {start_next:?}");
        }
    }
